...
def SAM(I1, I2):
    ...
    参数:
        I1: 第一个输入图像，形状为 (B, C, H, W)
        I2: 第二个输入图像，形状为 (B, C, H, W)

    返回:
        sam_values: 每个样本的 SAM 值，形状为 (B,)
    """
    # 确保输入形状一致
    assert I1.shape == I2.shape, "Input images must have the same shape"

    # 计算每个通道的平方和
    norm_I1 = torch.linalg.norm(I1.view(I1.size(0), I1.size(1), -1), dim=2)  # 使用 linalg.norm 替代 torch.norm
    norm_I2 = torch.linalg.norm(I2.view(I2.size(0), I2.size(1), -1), dim=2)  # 使用 linalg.norm 替代 torch.norm

    # 防止除零错误
    epsilon = 1e-10
    norm_I1 = torch.clamp(norm_I1, min=epsilon)
    norm_I2 = torch.clamp(norm_I2, min=epsilon)

    # 计算点积
    dot_product = (I1 * I2).view(I1.size(0), I1.size(1), -1).sum(dim=2)

    # 计算角度（弧度）
    cos_theta = dot_product / (norm_I1 * norm_I2)

    # 限制 cos_theta 的范围在 [-1, 1] 内，以防数值不稳定
    cos_theta = torch.clamp(cos_theta, -1.0, 1.0)

    # 使用反余弦函数计算角度，并转换为度数（可选）
    angle_rad = torch.acos(cos_theta)  # 弧度制

    return angle_rad.mean(dim=[-3,-2]) * (180.0 / torch.pi)  # 返回每个批次的平均值
...
def _qindex(img1, img2, block_size=16):  # 1 denoting the best fidelity to reference.
...
def D_lambda(img_fake, img_lm, block_size=16, p=1):
    """Spectral distortion
...
def D_s(img_fake, img_lm, pan, satellite='QuickBird', scale=16, block_size=8, q=1):
    """Spatial distortion
    img_fake, generated HRMS
    img_lm, LRMS
    pan, HRPan"""
...
def qnr(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=16, p=1, q=1, alpha=1, beta=1):
...
def ref_evaluate(pred, gt, scale=4, dynamic_range=1):
    ...
    return c_psnr, c_ssim, c_ergas, c_scc, c_rmse



def no_ref_evaluate(pred, pan, hs, scale=4,block_size = 128):
    # no reference metrics
    c_D_lambda = D_lambda(pred, hs, block_size=block_size)
    c_D_s = D_s(pred, hs, pan, scale=scale, block_size=block_size)
    c_qnr = qnr(pred, hs, pan, scale=scale, block_size=block_size)
    # c_qnr = Q(pan,hs,pred)
    # if c_D_lambda <=0.020 and c_D_s<=0.020:
    #     print('c_D_lambda',c_D_lambda)
    #     print('c_D_s', c_D_s)
    #     print('c_qnr', c_qnr)
